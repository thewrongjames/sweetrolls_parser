...
test('Max works amoungst other things', () => {
  const rootNode = parse('2 * 3, (max((6 * (6 + 3), -534) / 2), 1) + 5')
  expect(rootNode.run()).toEqual([[6], [32]])
})

test('Max fails with incorrectly structured arguments', () => {
  const rootNode = parse('max(6)')
  expect(rootNode.run()).toThrow(SweetRollsRuntimeError)
})

test('Basic min case works', () => {
  const rootNode = parse('min((5340 / 546, (14 / 10) * 3, 53), 2)')
...
test('Min works amoungst other things', () => {
  ...
  expect(rootNode.run()).toEqual([[3.9995]])
})
